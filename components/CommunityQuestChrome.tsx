...
export default function CommunityQuestChrome() {
  ...
  // Ultra-aggressive Chrome detection and CSS injection
  useEffect(() => {
    if (typeof window === 'undefined') return;

    // More aggressive Chrome detection
    ...
    if (isChrome) {
      console.log('Chrome detected - applying aggressive layout fixes');
      
      // Create multiple style elements for redundancy
      const createStyleElement = (id: string, css: string) => {
        ...
        style.setAttribute('data-chrome-fix', 'true');
        document.head.appendChild(style);
        return style;
      };

      // Primary Chrome fix
      createStyleElement('chrome-fix-primary', `
        .chrome-desktop-container {
      ...
      // Secondary Chrome fix (backup)
      createStyleElement('chrome-fix-secondary', `
        [data-chrome-layout="main"] {
          display: flex !important;
          flex-direction: row !important;
          width: 100% !important;
          height: 500px !important;
        }
        [data-chrome-layout="profile"] {
          width: 30% !important;
          height: 100% !important;
          flex-shrink: 0 !important;
        }
        [data-chrome-layout="levels"] {
          width: 70% !important;
          height: 100% !important;
          flex-shrink: 0 !important;
        }
      ...
      const forceChromeLayout = () => {
        ...
        profiles.forEach(selector => {
          const profile = document.querySelector(selector) as HTMLElement;
          if (profile) {
        ...
        const levels = [
          '.chrome-levels-container',
          '[data-chrome-layout="levels"]',
          '.chrome-levels-section'
        ];
        
        levels.forEach(selector => {
          const level = document.querySelector(selector) as HTMLElement;
          if (level) {
            level.style.cssText = `
              width: 70% !important;
              min-width: 70% !important;
        ...
        const leftCols = [
          '.chrome-levels-left',
          '[data-chrome-layout="left"]'
        ];
        
        leftCols.forEach(selector => {
          const leftCol = document.querySelector(selector) as HTMLElement;
          if (leftCol) {
        ...
        const rightCols = [
          '.chrome-levels-right',
          '[data-chrome-layout="right"]'
        ];
        
        rightCols.forEach(selector => {
          const rightCol = document.querySelector(selector) as HTMLElement;
          if (rightCol) {
      ...
      // More frequent reapplication for Chrome
      const interval = setInterval(forceChromeLayout, 25);
      
      // Watch for any DOM changes
      const observer = new MutationObserver(() => {
        setTimeout(forceChromeLayout, 10);
      });
      ...
      const handleResize = () => {
        setTimeout(forceChromeLayout, 50);
      };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Main Content with ULTRA-AGGRESSIVE CHROME LAYOUT */}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="space-y-6 sm:space-y-8">
          <div className="bg-card rounded-lg p-4 sm:p-6 shadow-lg border border-border">
            {/* ULTRA-AGGRESSIVE CHROME DESKTOP LAYOUT */}
            <div 
              className="chrome-desktop-container"
              data-chrome-layout="main"
              style={{
                display: 'flex',
                flexDirection: 'row',
            ...
            >
              ...
              <div 
                className="chrome-profile-container"
                data-chrome-layout="profile"
                style={{
                  width: '30%',
                  minWidth: '30%',
              ...
              <div 
                className="chrome-levels-container"
                data-chrome-layout="levels"
                style={{
                  width: '70%',
                  minWidth: '70%',
